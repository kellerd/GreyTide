...
function jqLiteDocumentLoaded(action, win) {
  win = win || window;
  if (win.document.readyState === 'Completed') {
...
var $AnimateProvider = ['$provide', function($provide) {
   ...
   *   must be called once the element animation is Completed. If a function is returned then the
  ...
  this.$get = ['$$q', '$$asyncCallback', '$rootScope', function($$q, $$asyncCallback, $rootScope) {
    ...
    function runAnimationPostDigest(fn) {
      ...
      $rootScope.$$postDigest(function ngAnimatePostDigest() {
        cancelFn = fn(function ngAnimateNotifyCompleted() {
          defer.resolve();
        });
      });
...
function Browser(window, document, $log, $sniffer) {
  ...
  self.$$CompletedOutstandingRequest = CompletedOutstandingRequest;
   ...
   */
  function CompletedOutstandingRequest(fn) {
    try {
  ...
  self.defer = function(fn, delay) {
    ...
    timeoutId = setTimeout(function() {
      ...
      CompletedOutstandingRequest(fn);
    }, delay || 0);
  ...
  self.defer.cancel = function(deferId) {
    if (pendingDeferIds[deferId]) {
      ...
      CompletedOutstandingRequest(noop);
      return true;
    }
    return false;
  };
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     * ```
     *
     * Completed list of shortcut methods:
     *
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      var jsonpDone = jsonpReq(url.replace('JSON_CALLBACK', 'angular.callbacks.' + callbackId),
        ...
        CompletedRequest(callback, status, callbacks[callbackId].data, "", text);
      ...
      var requestError = function() {
        ...
        CompletedRequest(callback, -1, null, null, '');
      };
    ...
    function CompletedRequest(callback, status, response, headersString, statusText) {
      // cancel timeout and subsequent timeout promise resolution
      timeoutId && $browserDefer.cancel(timeoutId);
      jsonpDone = xhr = null;

      callback(status, response, headersString, statusText);
      $browser.$$CompletedOutstandingRequest(noop);
    }
  };
 ...
 * of the deferred task when it Completeds.
 *
 * **Methods**
 *
 ...
 * can override it Completedly to change the behavior of `$sce`, the common case would
 ...
 *
 * That said, here's how you can Completedly disable SCE:
 *
 * ```
 * angular.module('myAppWithSceDisabledmyApp', []).config(function($sceProvider) {
 *   // Completedly disable SCE.  For demonstration purposes only!
...
function $$TestabilityProvider() {
  this.$get = ['$rootScope', '$browser', '$location',
     ...
     *
     * @description
     * Calls the callback when $timeout and $http requests are Completedd.
     *
...
function baseInputType(scope, element, attr, ctrl, $sniffer, $browser) {
  ...
  // or form autoCompleted on newer browser, we need "change" event to catch it
 ...
 *
 * `ngIf` differs from `ngShow` and `ngHide` in that `ngIf` Completedly removes and recreates the
